...
class Analysis:
    def __init__(self,dfg,dfp,record_range,iou_thresholds,verbose):
        ...
        # activity heat map
        act = self.dfg.lab_g.unique()
        act.sort()
        self.act_map = pd.DataFrame(0, index=act, columns=act)
    ...
    def activity_map(self,g1,i):
        y=self.dfp.loc[i, "lab_p"]
        for x in g1.iloc[:, 3]:
            self.act_map.loc[y,x]+=1
        if self.verbose:
            print(self.act_map.iloc[:,5:7])
    ...
    def main_loop(self):
        ...
        for i in r:
            ...
            ### Create Label Heat Map
            self.activity_map(g1,i)
        ...
        self.act_map.to_pickle('act_map.pkl')
        return
...
def charts(df,title):
    ...
    plt.bar(x, y1, color="royalblue",width=0.9)
    plt.bar(x, y2, bottom=y1, color="green",width=0.9)
    plt.bar(x, y3, bottom=y1 + y2, color="indigo",width=0.9)
    plt.bar(x, y4, color="gray",width=0.9)

    ### Bar Chart Value Labels
    for k in range(len(y_val)):
        plt.text(k-0.35,1*(y_max/25),"Count: "+str(y_val[k]),fontsize=8,color="yellow")
        plt.text(k - 0.35, 2*(y_max/25), "L/L: "+str(lli_val[k]),fontsize=8,color="yellow")
        if iou_val[k]!=0:
            plt.text(k - 0.35, 3*(y_max/25), "IoU:" +str(iou_val[k]),fontsize=8,color="yellow")
        if delta_val[k] != 0:
            plt.text(k - 0.35, 3*(y_max/25), "Delta: "+str(delta_val[k]),fontsize=8,color="yellow")

    ### Summary Statistics
    plt.text(4, y_max, 'Hits: '+str(hits)+"  "+str(hits2),fontsize=9,color="black")
    plt.text(4, y_max-1*(y_max/25), 'Misses: '+str(misses)+"  "+str(misses2), fontsize=9,color="black")
    plt.text(4, y_max-2*(y_max/25), 'No Ground Truths: '+str(nogts)+"  "+str(nogts2), fontsize=9,color="black")
    plt.text(4, y_max-3*(y_max/25), 'Overall Lead/Lag Index: ' + str(index), fontsize=9,color="black")
...
def activity_heatmap():
    data = pd.read_pickle("act_map.pkl")
    plt.figure(figsize=(68, 18))
    colormap = sns.color_palette("Greens")
    sns.heatmap(data,annot=True, linewidths=.5,cmap=colormap,cbar=False,annot_kws={"size":10},fmt="g")
    plt.title("ActionFormer", loc="center")
    ...
    plt.show()
    plt.draw()
    fig.savefig("fig_activity_map.png")
